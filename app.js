...
function setupJournal(){
  ...
  document.getElementById('mood-filter-row').querySelectorAll('.filter-pill').forEach(pill=>{
    pill.addEventListener('click',()=>{
      ...
      const filtered=f==='all'?state.entries:state.entries.filter(e=>e.mood===f);
      renderJournalEntries(filtered);
    });
  });
}
...
async function renderWeeklyInsight(){
  ...
  if(state.weeklyInsight){body.innerHTML=`<p style="color:var(--text2);font-size:.9rem;line-height:1.7">${state.weeklyInsight}</p>`;return;}
  body.innerHTML='<p class="insight-placeholder">Generating your weekly insight...</p>';
}
document.addEventListener('DOMContentLoaded',()=>{
...
async function generateWeeklyInsight(){
  ...
  const messages=[
    {role:'system',content:'You are Journea. Analyze these journal entries and provide a warm, insightful weekly summary in 2-3 short paragraphs. Identify: 1) dominant emotions and patterns, 2) recurring themes or triggers, 3) one gentle, actionable habit suggestion. Use non-clinical, supportive language. No bullet points — flowing prose only.'},
    {role:'user',content:`My journal entries this week:\n\n${combined}`},
  ];
  const insight=await callAzureAI(messages);
  state.weeklyInsight=insight; saveState();
  document.getElementById('weekly-insight-body').innerHTML=`<p style="color:var(--text2);font-size:.9rem;line-height:1.7">${insight.replace(/\n/g,'<br/>')}</p>`;
